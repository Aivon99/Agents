...
class HuggingFaceAdapter(ProviderAdapter):
    ...
    def _headers(self) -> dict[str, str]:
        if not settings.hf_api_key:
            raise ProviderAdapterError('Missing Hugging Face API key', error_code='missing_key')
        return {
    ...
    def parse_response(self, response: httpx.Response, route: RouteConfig) -> DispatchResult:
        if response.status_code >= 400:
            raise ProviderAdapterError('Hugging Face request failed', status_code=response.status_code, error_code='http_error')
        data = response.json()
    ...
    async def refresh_quota(self, route: RouteConfig) -> dict[str, Any] | None:
        # HF documents monthly credits, but not a strong per-key remaining-free-quota endpoint for this use case.
        return None
